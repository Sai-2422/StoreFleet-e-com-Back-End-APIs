...
export const getAllProducts = async (req, res, next) => {
  try {
    ...
    // Price Range Query
    if (price) {
      const { gte: priceGte, lte: priceLte } = price;
      if (priceGte && !isNaN(parseFloat(priceGte))) {
        filterQuery.price = { $gte: parseFloat(priceGte) };
      }
      if (priceLte && !isNaN(parseFloat(priceLte))) {
        filterQuery.price = {
          ...filterQuery.price,
          $lte: parseFloat(priceLte),
        };
      }
    }
    ...
    // Rating Range Query
    if (rating) {
      const { gte: ratingGte, lte: ratingLte } = rating;
      if (ratingGte && !isNaN(parseFloat(ratingGte))) {
        filterQuery.rating = { $gte: parseFloat(ratingGte) };
      }
      if (ratingLte && !isNaN(parseFloat(ratingLte))) {
        filterQuery.rating = {
          ...filterQuery.rating,
          $lte: parseFloat(ratingLte),
        };
      }
    }
    ...
    let matchedProducts;
    if (keyword) {
      matchedProducts = await findProductByFilter(
    ...
    return res
      .status(200)
      .json({ status: "success", response: matchedProducts });
  } catch (error) {
    return res
      .status(500)
      .json({ status: "error", response: "Internal server error" });
  }
};
